...
struct LabelParams {
    /**
     * @brief   Constructor
     * @param   pColumnNameIdx        specifies the zero-based row index of the column labels,
     * setting it to -1 prevents column lookup by label name, and gives access to all rows as
     * document data. Default: 0
     * @param   pRowNameIdx           specifies the zero-based column index of the row labels,
     * setting it to -1 prevents row lookup by label name, and gives access to all columns as
     * document data. Default: -1
     */
...
struct SeparatorParams {
     ...
     * @param   pHasCR                specifies whether a new document (i.e. not an existing
     * document read) should use CR/LF instead of only LF (default is to use standard behavior of
     * underlying platforms - CR/LF for Win, and LF for others).
     * @param   pQuotedLinebreaks     specifies whether to allow line breaks in quoted text (default
     * false)
     * @param   pAutoQuote            specifies whether to automatically dequote data during read,
     * and add quotes during write (default true).
     */
...
class Document {
   public:
     ...
     * @param   pPath                 optionally specifies the path where the CSV-file will be
     * created (if not specified, the original path provided when creating or loading the Document
     * data will be used).
     */
    ...
    void WriteCsv() const {
        ...
        if (mIsUtf16) {
            ...
            if (mIsLE) {
                ...
                wstream.imbue(
                    std::locale(wstream.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff>));
            }
